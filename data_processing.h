...
namespace processing {
    ...
    bool edit_user() {
        ...
        cout << "请输入需要修改用户的用户名: ";
        cin >> getUsername;
        std::string userInfo = user_service.get_user_by_username(getUsername);
        if (!userInfo.empty()) {
            int check_edit = 0;
            // 是否确认修改
            cout << "是否确认修改员工信息？(1 是，0 不是）：";
            cin >> check_edit;
            if (!check_edit) {
            ...
            cout << "========== edit_user ==========" << endl;
            cout << "您正在进行员工 [" << userInfo << "] 信息的修改;" << endl;
            user_edit_vo.user_number = userInfo;
            cout << "请输入员工的姓名：";
            cin >> user_edit_vo.user_name;
            cout << "请输入员工的性别：";
            cin >> user_edit_vo.user_sex;
            cout << "请输入员工的部门：";
    ...
    bool delete_user() {
        ...
        cout << "========== delete_user ==========" << endl;
        service::user user_service;
        std::string userInfo = user_service.get_user_by_username(userName);
        if (!userInfo.empty()) {
            int check_delete = 0;
            // 是否确认删除
            ...
            // 开始删除员工信息
            return user_service.delete_user(userInfo);
        } else {
            cout << "未找到员工信息！" << endl;
            return false;
        }
